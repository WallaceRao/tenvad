...
def _adjust_audio_shorter(audio_data: np.ndarray, sample_rate: int, target_duration: float, 
    ...
    if (total_duration - target_duration) > 0.1:
       target_duration = target_duration + 0.1
...
def _adjust_audio_longer(audio_data: np.ndarray, sample_rate: int, target_duration: float, 
    ...
    if target_duration < 1.4:
        ...
        tail_padding_samples = int(padding_duration * sample_rate * 2.0 / 3.0)
    ...
    if current_duration < target_duration:
        ...
        head_padding = np.zeros(head_padding_samples, dtype=audio_data.dtype)
        tail_padding = np.zeros(tail_padding_samples, dtype=audio_data.dtype)
        reconstructed_audio = np.concatenate((head_padding, reconstructed_audio, tail_padding))    
    return reconstructed_audio
...
async def adjust_audio_length_endpoint(
    ...
    try:
        ...
        # 执行VAD检测获取人声段
        voice_segments = detect_voice_segments(audio_data, sample_rate)
        #logger.info(f"voice_segments:{voice_segments}")
        # 调整音频长度
